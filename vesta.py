...
def cli(argv: list[str] | None = None) -> int:
    ...
    def add_common(p: argparse.ArgumentParser) -> None:
        p.add_argument("--profile", choices=sorted(PROFILES), default="flagship", help="Board profile (default: %(default)s)")
        p.add_argument("--template", choices=["auto", "text", "kv", "data", "table", "metrics"], default="auto", help="Rendering template; 'auto' detects from input shape (default: %(default)s)")
        p.add_argument("--visible-spaces", action="store_true", help="Show spaces as · in terminal preview")
        p.add_argument("--cell-width", type=int, default=2, help="Terminal preview width per board cell (default: %(default)s)")
        p.add_argument("--no-ansi", action="store_true", help="Disable ANSI color in terminal preview")
        p.add_argument("--title", metavar="TEXT", help="Title row rendered above content with colored bookend tiles")
    ...
    add_common(cloud_p)
    cloud_p.add_argument("--token", default=os.getenv("VESTABOARD_TOKEN"), help="Cloud RW API token (falls back to VESTABOARD_TOKEN env var)")
    ...
    add_common(local_p)
    local_p.add_argument("--api-key", default=os.getenv("VESTABOARD_LOCAL_API_KEY"), help="Local API key (falls back to VESTABOARD_LOCAL_API_KEY env var)")
    local_p.add_argument("--host", default=os.getenv("VESTABOARD_LOCAL_HOST", "http://vestaboard.local:7000"), help="Local API host (falls back to VESTABOARD_LOCAL_HOST env var, default: %(default)s)")
    local_p.add_argument("--strategy", choices=["column", "reverse-column", "edges-to-center", "row", "diagonal", "random"], metavar="STRATEGY", help="Board animation strategy: column, reverse-column, edges-to-center, row, diagonal, random")
    local_p.add_argument("--step-interval-ms", type=int, metavar="MS", help="Delay in milliseconds between animation steps")
    local_p.add_argument("--step-size", type=int, metavar="N", help="Number of rows/columns to animate per step (default: 1)")
    ...
    read_cloud_p.add_argument("--visible-spaces", action="store_true", help="Show spaces as · in terminal preview")
    read_cloud_p.add_argument("--cell-width", type=int, default=2, help="Terminal preview width per board cell (default: %(default)s)")
    read_cloud_p.add_argument("--no-ansi", action="store_true", help="Disable ANSI color in terminal preview")
